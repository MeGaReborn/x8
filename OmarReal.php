...
sendMessage($chatId, "اهلا 💡بك عزيزي \n
	...
	" 
	 ...
	 if($message == "هلو" or $message == "مرحبا" or $message == "هلاو" or $message == "السلام عليكم"){
	 	sendmark($chatId, "نورت حبي 🌝☘ ",$memb);
	 }
}
...
sendmark($chatId, "وداعا عزيزي 🌝☘ : " . "[$nam](https://t.me/$user",$memb);
    ...
    file_get_contents($url);
     ...
     function kick($chatId,$for,$reply){
  
    $url = $GLOBALS[website]."/kickChatMember?chat_id=".$chatId."&user_id=".$for."&text=".urlencode($message);
    file_get_contents($url);
    } 
     
     function rekick ($chatId, $for, $memb){
    $url = $GLOBALS[website].'/kickChatMember?chat_id='.$chatId.'&user_id='.$for.'&text='.urlencode($message).'&reply_to_message_id='.$memb.'&disable_web_page_preview=true';
    file_get_contents($url);
     } 
